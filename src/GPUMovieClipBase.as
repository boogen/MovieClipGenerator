    ...
    public class GPUMovieClipBase {
        ...
        public function fromMC(mc:MovieClip):void {
            ...
            for (var i:int = 0; i < mc.totalFrames; ++i) {
                ...
                mc.addFrameScript(i, function():void { trace(mc); } );
                mc.gotoAndStop(i + 1);
                mc.gotoAndStop(i);
		...
		public function flatten(assets:Vector.<Object>):void 
		{
			if (bmp) {
				assets.push( { "bitmap":bmp, "name":name } );	
			}
		...
		public function dumpLikeABoss(parentXML:XML, spritesheetName:String):void 
		{
			var assets:Vector.<Object> = new Vector.<Object>();
			flatten(assets);			
			var spritesheet:Spritesheet =  new Spritesheet();
			var result:Object = spritesheet.generate(assets);
			var xml:String = serializeAnimation(result.animation);			
			
			var spritesheetXML:XML = new XML(xml);
			parentXML.appendChild(spritesheetXML);
			
			var animation:XML = <animation />			
			writeMovieClip(animation);			
			parentXML.appendChild(animation);
						
		//*	
			var bArray:ByteArray = PNGEncoder.encode(result.bitmapData);
			
			var file:File = File.applicationStorageDirectory.resolvePath(spritesheetName);
        ...
        public static function prepareGPUStaticData(ctx:Context3D):void {
            ...
            vsAsm.assemble(Context3DProgramType.VERTEX,
            ([
                "mul vt0, vc6.zw, va0.xy",
            ...
            fsAsm.assemble(Context3DProgramType.FRAGMENT, ([
                "tex oc, v0, fs0 <2d, norepeat, linear, nomip>"
            ]).join("\n"), true);
